...
class KhipuService(object):
    ...
    def do_hash(self):
        ...
        """
        return hmac.new(self.secret.encode(), self.concatenated(), hashlib.sha256).hexdigest()

    def concatenated(self):
        """
        El orden de los valores debe ser METODO&URL&LOS_PARAMETROS_A_ENVIAR
        """
        cad = "&".join(['%s=%s' % (quote(str(k).encode(), safe=''), quote(str(v).encode(), safe='')) for k, v in self.data.items()])  # noqa
        cad = "&" + cad if cad else ''
        cad = '{}&{}'.format(self.method, quote(self.get_url_service(), safe='')) + cad
        return cad.encode()
